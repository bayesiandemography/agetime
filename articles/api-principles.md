...
| `*_standard()` | Convert labels to standard format | Same shape as `x` |
| `*_modify()` | Regroup labels into new intervals | Same shape as `x` |
...
Arguments beginning with `x_` describe how input labels in `x` should be
parsed. They do not describe how newly generated output labels should be
rendered.

For example,
[`period_standard()`](https://bayesiandemography.github.io/agetime/reference/period_standard.md)
uses `x_multi` to interpret the input:

``` r

period_standard("2020-2025", x_multi = "exclude")
#> [1] "2020-2026"
```

Arguments beginning with `label_` describe how new output labels should
be rendered. For example,
[`period_labels_ten()`](https://bayesiandemography.github.io/agetime/reference/period_labels.md)
uses `label_multi` to decide whether the displayed upper limit is
...
return values with the same length as `x`. If `x` is a factor, they also
update levels.
...
can add a `70+` level even when no value in `x` currently belongs to
that open group.

``` r

x <- c("0-4", "60-64")
age_levels_set_open(x, lower_open = 70)
#> [1] 0-4   60-64
#> Levels: 0-4 60-64 70+
```
