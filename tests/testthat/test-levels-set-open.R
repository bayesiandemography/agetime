test_that("age_set_open_right() relabels groups at or above lower_open", {
  x <- c("20-24", "80-84", "100+")

  ans <- age_set_open_right(x, lower_open = 80)
  expect_s3_class(ans, "factor")
  expect_identical(as.character(ans), c("20-24", "80+", "80+"))
  expect_identical(levels(ans), c("20-24", "80+"))

  ans <- age_set_open_right(x, lower_open = 50)
...
test_that("age_set_open_right() can add an unobserved open level", {
  ans <- age_set_open_right(c("0-4", "60-64"), lower_open = 70)
...
test_that("age_set_open_right() relabels factor levels", {
  ...
  ans <- age_set_open_right(fx, lower_open = 80)
...
test_that("age_set_open_right() errors when lower_open splits an interval", {
  expect_error(
    age_set_open_right(c("70-89", "80-84"), lower_open = 80),
...
test_that("age_set_open_right() relabels already-open groups", {
  expect_identical(
    as.character(age_set_open_right("100+", lower_open = 100)),
    "100+"
  )
  expect_identical(
    as.character(age_set_open_right("100+", lower_open = 80)),
    "80+"
  )
})
...
test_that("cohort_set_open_left() errors when upper_open splits", {
  expect_error(
    cohort_set_open_left(c("2010-2019", "2015"), upper_open = 2015),
...
test_that("cohort_set_open_right() relabels groups to right-open cohorts", {
  ...
  ans <- cohort_set_open_right(x, lower_open = 2030)
  expect_s3_class(ans, "factor")
...
test_that("period_set_open_left() relabels groups to left-open periods", {
  ...
  ans <- period_set_open_left(x, upper_open = 2020)
  expect_s3_class(ans, "factor")
...
test_that("period_set_open_right() relabels groups to right-open periods", {
  ...
  ans <- period_set_open_right(x, lower_open = 2030)
  expect_s3_class(ans, "factor")
  expect_identical(as.character(ans), c("2020-2024", "2025-2029", "2030+"))
  expect_identical(levels(ans), c("2020-2024", "2025-2029", "2030+"))
})
