test_that("age_sort() sorts levels without changing observed values", {
...
test_that("age_sort() converts character input to a sorted factor", {
  x <- c("25-29", "0-4", "5-9")
  ans <- age_sort(x)
...
test_that("age_sort() with decreasing = TRUE sorts levels in reverse", {
  x <- factor(c("20-24", "0-4", "5-9"), levels = c("20-24", "0-4", "5-9"))
  ans <- age_sort(x, decreasing = TRUE)

  expect_identical(as.character(ans), c("20-24", "0-4", "5-9"))
  expect_identical(levels(ans), c("20-24", "5-9", "0-4"))
})

test_that("age_sort() puts NA second-to-last and Total last", {
...
test_that("age_sort() leaves a single level unchanged", {
  x <- factor("0-4")
  ans <- age_sort(x)
...
test_that("period_sort() and cohort_sort() sort levels", {
  x <- c("2025-2030", "2020-2025")
  expected <- c("2020-2025", "2025-2030")
  expect_identical(levels(period_sort(x)), expected)
  expect_identical(levels(cohort_sort(x)), expected)
  expect_identical(as.character(period_sort(x)), x)
  expect_identical(as.character(cohort_sort(x)), x)
})
