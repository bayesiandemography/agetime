...
test_that("age_set_open_right() with length-0 input adds an open level", {
  expect_identical(
    age_set_open_right(character(0), lower_open = 65),
    factor(character(0), levels = "65+")
  )
  expect_identical(
    age_set_open_right(factor(), lower_open = 65),
    factor(character(0), levels = "65+")
  )
})
...
test_that("age_set_open_right() adds a level when no labels qualify", {
  expect_identical(
    age_set_open_right("80-84", lower_open = 90),
...
test_that("age_set_open_right() with length-0 factor relabels levels", {
  ...
  out <- age_set_open_right(fx, lower_open = 80)
  expect_identical(
    out,
    factor(levels = levels(age_set_open_right(f_ref, lower_open = 80)))
  )
...
test_that("cohort_set_open_left() with length-0 input adds an open level", {
  expect_identical(
    cohort_set_open_left(character(0), upper_open = 2020),
...
test_that("cohort_set_open_left() with length-0 factor relabels levels", {
  ...
  fx <- factor(character(0), levels = lev)
  f_ref <- factor(lev, levels = lev)
  out <- cohort_set_open_left(fx, upper_open = 2025)
  expect_identical(
    out,
    factor(levels = levels(cohort_set_open_left(f_ref, upper_open = 2025)))
  )
  expect_identical(length(out), 0L)
