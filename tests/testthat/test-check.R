...
expect_check <- function(val, check, passed, ok = NULL) {
  if (is.null(ok)) {
    ok <- isTRUE(passed)
  }
...
test_that("valid_life passes for life-table labels even when there is a gap", {
  ...
  expect_check(val, "valid_life", TRUE)
...
test_that("valid_life check fails for non-life-table age groups", {
  ...
  expect_check(val, "valid_life", FALSE)
})
