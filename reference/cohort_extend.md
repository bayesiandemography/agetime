...
Add `n` cohorts to an existing set of labels `x`. The width of the
cohorts is derived from the `width` argument, or from the width of the
last label in `x`.
...
- n:
  ...
  The number of cohorts to add. Default is `1`.
...
If `x` is a factor, `cohort_extend` returns a factor; otherwise it
